...
The website is generated using [Material for MkDocs](https://squidfunk.github.io/mkdocs-material) and can be viewed at [homelab.kube.localhost](https://homelab.kube.localhost).
...
<!-- - Find the TXT record for `homelab.kube.localhost` and switch the `ownerID` between `homelab` and `horus` -->
